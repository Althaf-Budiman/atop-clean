...
@section('main')
    ...
    <div class="p-3">
        <form action="{{ url('/search') }}" method="GET">
            <input type="search" placeholder="Cari Laundry Berdasarkan Nama Laundry" value=""
                class="block w-full input input-primary" name="search">
        </form>
    </div>
    {{-- End Of Search Bar --}}

    <h1 class="text-3xl text-gray-600 ms-5 font-bold">Manajemen Laundry Sepatu, Tas, dan Topi.</h1>
    <div class="flex p-5 flex-wrap justify-center gap-10 flex-col md:flex-row">
        @foreach ($laundries as $laundry)
            ...
            <label for="modal_detail_{{ $laundry->id }}">
                <div class="card card-compact shadow-lg w-64 overflow-hidden">
                    <div class="hover:cursor-pointer hover:opacity-80 hover:bg-gray-100 transition">
                        ...
                        @else
                            <div
                                class="h-40 w-full object-cover bg-gray-500 text-black font-semibold text-center leading-[10rem]">
                                No Image</div>
                        @endisset
                        <div class="card-body">
                            <h2 class="text-xl font-semibold whitespace-nowrap text-ellipsis overflow-hidden">
                                {{ $laundry->nama }}</h2>
                            <div class="flex">
                                <p class="whitespace-nowrap text-ellipsis overflow-hidden">Customer:
                            ...
                            <div class="flex">
                                <p class="font-medium text-xl text-green-600"><b>Rp.
                                        {{ number_format($laundry->harga, 0, '.', '.') }}</b></p>
                            </div>
                        </div>
                    </div>
                    {{-- Button Done --}}
                    <label for="modal_done_{{ $laundry->id }}">
                        <div
                            class="bg-green-600 text-white font-semibold text-center text-5xl h-14 w-full object-cover hover:cursor-pointer hover:opacity-80 transition">
            ...
            <div class="modal">
                <div class="modal-box">
                    <h3 class="text-lg font-bold text-center">Apakah Anda Yakin Sudah Menyelesaikan "{{ $laundry->nama }}"?
                    </h3>
                ...
                <div class="modal-box">
                    <div class="flex flex-col md:flex-row gap-5">
                        <div class="flex flex-col">
                            ...
                            @else
                                <div
                                    class="h-56 w-48 object-cover bg-gray-500 text-black font-semibold text-center leading-[14rem] rounded-lg">
                                    No Image</div>
                            @endisset
